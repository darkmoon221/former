...
export class AutocompleteElementComponent
  extends BaseElementComponent<AutoCompleteElement>
    ...
    if (this.field.filter) {
      ...
      if (this.field.field) {
        ...
        for (let i = 0; i < (this.suggestions as any[]).length; i++) {
          let suggestion = (this.suggestions as any[])[i];
          if (
            suggestion.toLowerCase().toLowerCase().includes(query.toLowerCase())
          ) {
            filtered.push(suggestion);
          }
        }
    ...
    if (this.field.options) {
      Object.keys(this.field.options).forEach(option => {
        if (this.elementRef?.hasOwnProperty(option)) {
          (this.elementRef as any)[option] = (this.field.options as any)[
            option as keyof AutoCompleteOptions
          ];
        }
      });
    }
